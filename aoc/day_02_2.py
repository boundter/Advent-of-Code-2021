...
class Position:
    ...
    def forward(self, scalar: int) -> "Position":
        return Position(self.x + scalar, self.y + self.aim * scalar, self.aim)

    def down(self, scalar: int) -> "Position":
        return Position(self.x, self.y, self.aim + scalar)

    def up(self, scalar: int) -> "Position":
        return self.down(-scalar)
...
def get_position(commands: typing.Iterable[str]) -> Position:
    def apply_command(previous_pos: Position, command: str) -> Position:
        directions = {
            "forward": previous_pos.forward,
            "down": previous_pos.down,
            "up": previous_pos.up,
        }
        split = command.split(" ")
        return directions[split[0]](int(split[1]))

    return reduce(
        lambda pos, command: apply_command(pos, command),
        commands,
        Position(0, 0, 0),
    )
