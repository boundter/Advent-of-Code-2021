...
class BingoBoard:
    ...
    tipped: typing.Set[int] = field(init=False, default_factory=set)
    ...
    def __post_init__(self):
        ...
        self.cols = list(map(set, transpose(self.board)))

    def tip(self, number: int):
        self.tipped.add(number)
        list(map(lambda r: r.discard(number), self.rows))
        list(map(lambda c: c.discard(number), self.cols))
...
def find_winning_board(numbers: typing.List[int], boards: typing.List[BingoBoard]):
    def check_winner(num: int):
        list(map(lambda b: b.tip(num), boards))
        return any(map(lambda b: b.bingo(), boards))

    winning_number = first_true(numbers, pred=lambda num: check_winner(num))
    board = first_true(boards, pred=lambda b: b.bingo())
    untipped = board.untipped()
    return winning_number * sum(untipped)
...
def main():
    ...
    print(find_winning_board(numbers, boards))
