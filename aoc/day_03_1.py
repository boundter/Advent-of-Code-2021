...
def power_consumption(report: typing.Iterable[str]) -> int:
    length = len(report)
    # convert the strings to ints and transpose the list
    ints = map(lambda x: map(int, iter(x)), report)
    transpose = map(list, zip(*ints))
    # The sum of the numbers along the long axis should be bigger than half its
    # length for 1 to be the most occuring bit.
    sums = map(sum, transpose)
    gamma_binary = list(map(lambda x: int(x > length / 2.0), sums))
    gamma = int("".join(map(str, gamma_binary)), 2)
...
def main():
    ...
    print(power_consumption(inp.splitlines()))
